...
import { Download, Award, CheckCircle2, Scale, Printer, ArrowLeft } from 'lucide-react';
...
export const ReportView: React.FC<ReportViewProps> = ({ report, onBack, onExport }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-10 pt-4 px-4 overflow-y-auto h-full custom-scrollbar">
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 relative overflow-hidden">
        ...
        <div className="flex justify-between items-start relative z-10">
          ...
          <div className="text-right">
            ...
            <div className="mt-2 inline-flex items-center gap-1 bg-green-50 text-green-700 px-2 py-1 rounded text-xs font-medium border border-green-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-500 mt-1 flex items-start gap-1">
                        <span className="font-semibold text-primary-600 uppercase text-[10px] tracking-wide px-1.5 py-0.5 rounded bg-primary-50">
                          {item.category}
                        </span>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <button onClick={onExport} className="w-full mt-6 flex items-center justify-center gap-2 bg-slate-900 text-white py-2.5 rounded-lg hover:bg-slate-800 transition-colors text-sm font-medium">
              <Download className="w-4 h-4" />
              Zertifikat als PDF exportieren
            </button>
          </div>
        </div>
