...
export class CostCalculatorService {
  ...
  calculateProBonoValue(
    ...
    const beneficiariesFactor = 1 + (Math.log10(totalBeneficiaries) * 0.2);
  ...
  evaluateFormula(formula: string, variables: Record<string, number>): number {
    // Einfacher Formel-Parser für mathematische Ausdrücke
    ...
    // Sichere Evaluation (nur Zahlen und Operatoren erlaubt)
    if (!/^[\d\s+\-*/().]+$/.test(expression)) {
      throw new Error(`Unsichere Formel: ${expression}`);
    }
    ...
    try {
      ...
      throw new Error(`Formel-Fehler: ${expression} - ${error}`);
    }
  }
  ...
  /**
   * Validiert Kostenberechnung
   */
  validateCostCalculation(result: CostCalculationResult): CostValidationResult {
    const errors: CostValidationResult['errors'] = [];
    const warnings: CostValidationResult['warnings'] = [];
    const suggestions: string[] = [];

    // Prüfe ob Summen korrekt
    const calculatedSubtotal = result.totalMaterial + result.totalPersonnel + 
                                result.totalOperational + result.totalMiscellaneous;

    if (Math.abs(calculatedSubtotal - result.subtotal) > 0.01) {
      errors.push({
        field: 'subtotal',
        message: `Zwischensumme stimmt nicht: Berechnet ${calculatedSubtotal}, gespeichert ${result.subtotal}`,
        severity: 'Error'
      });
    }

    // Prüfe Mindest-Stundensätze für Personnel
    if (this.options.enforceMinimumCosts && this.options.minimumHourlyRate) {
      result.personnelCosts.forEach(item => {
        if (item.unit === 'Stunden' && item.unitPrice < this.options.minimumHourlyRate!) {
          warnings.push({
            field: `personnelCosts.${item.id}.unitPrice`,
            message: `Stundensatz ${item.unitPrice} € unter JVEG-Minimum`,
            currentValue: item.unitPrice,
            recommendedValue: this.options.minimumHourlyRate,
            reason: `JVEG M1 Mindestsatz: ${this.options.minimumHourlyRate} €/h`
          });
        }
      });
    }

    // Prüfe Verhältnismäßigkeit
    if (result.economicViability && !result.economicViability.isEconomicallyViable) {
      suggestions.push(
        'Wirtschaftlichkeit fraglich - Pro-Bono-Prüfung empfohlen',
        result.economicViability.recommendation
      );
    }

    // Prüfe ob Faktoren angewendet wurden
    if (result.appliedFactors.length === 0) {
      suggestions.push('Keine Faktoren angewendet - prüfen Sie ob Qualitäts-, Risiko- oder Komplexitätsfaktoren relevant sind');
    }
  ...
  private generateMethodologyDescription(items: CostItem[]): string {
    ...
    const formulas = new Set(items.map(i => i.formula?.id).filter(Boolean));
